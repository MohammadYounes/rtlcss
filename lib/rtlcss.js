      ...
      if (prevent === false) {
        switch (node.type) {
          ...
          case 'comment':
            state.parse(node, function (current) {
              ...
              if (current.directive === null) {
                ...
                context.util.each(context.config.plugins, function (plugin) {
                  current.directive = plugin.control[current.metadata.name]
            ...
            break
          case 'decl':
            // if each was broken by a matching value directive .. break
            if (!context.util.each(context.config.plugins,
                function (plugin) {
                  return context.util.each(plugin.value, function (directive) {
            ...
            // loop over all plugins/value directives
            context.util.each(context.config.plugins, function (plugin) {
              return context.util.each(plugin.property, function (directive) {
                if (node.prop.match(directive.expr)) {
                  var raw = context.config.preserveComments && node.raws.value && node.raws.value.raw ? node.raws.value.raw : node.value
                  var pair
                  if (context.config.preserveComments) {
                    var commentsLessRawValue = context.util.saveComments(raw)
                    pair = directive.action(node.prop, commentsLessRawValue, context)
                    pair.value = context.util.restoreComments(pair.value)
                  } else {
                    pair = directive.action(node.prop, raw, context)
                  }
