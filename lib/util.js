function main (configuration) {
  ...
  var REGEX_TOKEN = new RegExp(TOKEN_CHARACTER + '(\\d+)', 'i')
  var REGEX_COMPLEMENT = new RegExp('(calc' + TOKEN_CHARACTER + '\\d+)|(\\-?(\\d*?\\.\\d+|\\d+))(?!d\\()', 'i')
  var REGEX_NEGATE_ONE = new RegExp('(calc' + TOKEN_CHARACTER + '\\d+)|(\\-?(\\d*?\\.\\d+|\\d+))(?!d\\()', 'i')
  var REGEX_NEGATE_ALL = new RegExp('(calc' + TOKEN_CHARACTER + '\\d+)|(\\-?(\\d*?\\.\\d+|\\d+))(?!d\\()', 'ig')
  ...
  var util = {
    ...
    logAtRuleAction: function (rule, directive) {
      this.log('AtRule:\t%s', info(rule.name))
    ...
    restoreFunctions: function (result) {
      while (REGEX_TOKEN.test(result.value)) {
        result.value = result.value.replace(REGEX_TOKEN, function (m, i) {
          return result.store[i - 1]
        })
      }
      return result.value
    },
    ...
    isLastDecl: function (decl) {
      var nodes = decl.parent.nodes.filter(function (node) {
        return node.type === decl.type
      })
      return nodes[nodes.length - 1] === decl
    }
  }
  return util
}
