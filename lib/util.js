function main (configuration) {
  ...
  var util = {
    ...
    logAtRuleAction: function (atRule, directive) {
      this.log('AtRule:\t%s', info(atRule.name))
      this.log('Action:\t%s', success(directive.name))
      this.logSource(atRule.source)
    },
    logRuleAction: function (rule, directive) {
      this.log('Rule:\t%s', info(rule.selector))
      this.log('Action:\t%s', success(directive.name))
      this.logSource(rule.source)
    },
    logDeclAction: function (decl, directive) {
      this.log('Decl:\t%s', info(decl.prop))
      this.log('Action:\t%s', success(directive.name))
      this.logSource(decl.source)
    },
    logPropAction: function (decl, result, directive) {
      this.log('Prop:\t%s', info(decl.prop))
      this.log('Action:\t%s', success('flip ' + directive.name))
      this.logSource(decl.source)
    },
    logSource: function (source) {
      if (source) {
        this.log('Source:\t%s\t%s:%s', info(source.file), info(source.start.line), info(source.start.column))
      }
    },
    ...
    restoreComments: function (value) {
      var self = this
      var index = 0
      var result = value.replace(REGEX_REPLACEMENT_CHARACTER_PLACEHOLDER, function () { return self.comments[index++] })
    ...
    unguard: function (state, callback) {
      ...
      if (state.indexed === true) {
        var detokenizer = new RegExp(state.token + '(\\d+)', 'i')
        while (detokenizer.test(state.value)) {
          state.value = state.value.replace(detokenizer, function (m, i) {
            var item = state.store[i - 1]
            callback(item)
            return item
          })
        }
        ...
        return state.value.replace(state.token, function (m, i) {
          var item = state.store.shift()
          callback(item)
          return item
        })
      }
    ...
    regex: function (what, options) {
      ...
      for (var x = 0; x < what.length; x++) {
        switch (what[x]) {
          case 'percent':
            expressions.push('(' + PATTERN_NUMBER + '%)')
            break
          case 'length':
            expressions.push('(' + PATTERN_NUMBER + ')(?:ex|ch|r?em|vh|vw|vmin|vmax|px|mm|cm|in|pt|pc)?')
            break
          case 'number':
            expressions.push('(' + PATTERN_NUMBER + ')')
            break
          case 'position':
            ...
            break
          case 'calc':
            expressions.push('(calc' + PATTERN_PLACEHOLDER + '\\d+)')
            break
        }
      }
